...
jQuery(document).ready(function($) {
	...
	var checkSlashes = path.split("/").length - 1;
	if (path != "/" && checkSlashes > 1) {
		$('.main-navigation a[href^="' + path.substring(0, path.lastIndexOf('/') + 1) + '"]').parent().addClass('active');
	}	else if(path.length > 1) $('.main-navigation a[href ^= "' + path + '"]').parent().addClass('active');
		/**
		 * SENDMAIL
		 *
		 */
	...
	$('a[id^="fav-id-"]').on('click', function(e) {
		e.preventDefault();
		var count = Number($(".favorites_count").text());
		if (e.target.outerHTML.indexOf("minus") !== -1) {
			$(".favorites_count").text(count - 1);
		} else {
			$(".favorites_count").text(count + 1);
		}
	});
	if ($('.pagination span').children('a').length > 0) {
		$('.pagination span').addClass('active');
	}
	$('.btn.sale-buy').addClass('add-to-cart');
	...
	if ($('.page-preloading').length) {
		$('a:not([href^="#"])').on('click', function(e) {
			if ($(this).attr('class') !== 'video-popup-btn' && $(this).attr('class') !== 'ya-share2__link' && $(this).attr('class') !== 'ajax-post-link' && $(this).attr('class') !== 'read-more ajax-post-link' && $(this).attr('class') !== 'nt-open') {
				e.preventDefault();
	...
	if (typeof window.Modernizr !== "undefined" && !Modernizr.touch) {
		$(window).on('load resize', function() {
			$('.all').css({
				minHeight: $(window).height() - $('footer.footer').height()
			});
			closeFilterPane();
		});
		$(window).on('scroll', function() {
	...
	$("#useredit").on("click", function(e) {
		e.preventDefault();
		$(".useredit").slideToggle(500);
	});
	var widthHalf = $(window).width() / 2;
	$(".slick-track>.slick-slide").each(function() {
		var slide = $(this).attr("id"),
			cls = "." + slide;
		window["check_" + slide] = window["go_" + slide] = window["status_" + slide] = window["line_g_" + slide] = window["d_" + slide] = false, window["snap_" + slide] = Snap(cls), window["poly_" + slide] = window["snap_" + slide].select('.masking>path'), window["lines_" + slide] = window["snap_" + slide].select('.lines>line'), window["lines2_" + slide] = window["snap_" + slide].select('.lines2>line');
		$(cls).on("mouseenter mouseleave", function(elem) {
			if (elem.type == "mouseenter") {
				window["go_" + slide] = true;
			} else {
				window["d_" + slide] = "M0,0 90,0 70,65 0,65z";
				window["line_g_" + slide] = {
					x1: 90,
					x2: 70
				};
				window["check_" + slide] = true;
				window["go_" + slide] = window["status_" + slide] = false;
			}
		});
		$(cls).on("mousemove", function(e) {
			if (window["go_" + slide] == true) {
				if (e.pageX >= widthHalf) {
					if (window["status_" + slide] != "right") {
						window["d_" + slide] = "M0,0 70,0 50,65 0,65z";
						window["line_g_" + slide] = {
							x1: 70,
							x2: 50
						};
						window["status_" + slide] = "right";
						window["check_" + slide] = true;
					}
				} else {
					if (window["status_" + slide] != "left") {
						window["d_" + slide] = "M0,0 110,0 90,65 0,65z";
						window["line_g_" + slide] = {
							x1: 110,
							x2: 90
						};
						window["status_" + slide] = "left";
						window["check_" + slide] = true;
					}
				}
			}
		});
		window["timer_" + slide] = setInterval(function() {
			if (window["check_" + slide]) {
				console.log(window["check_" + slide]);
				window["check_" + slide] = false;

				window["lines_" + slide].animate({ strokeWidth: 25 }, 550, mina.easeinout, function() {
					window["lines_" + slide].animate({ strokeWidth: 3 }, 550, mina.easeinout, function() {});
				});
				window["lines2_" + slide].animate({ strokeWidth: 15 }, 650, mina.easeinout, function() {
					window["lines2_" + slide].animate({ strokeWidth: 3 }, 650, mina.easeinout, function() {});
				});

				window["lines_" + slide].animate(window["line_g_" + slide], 1100, mina.easeinout, function() {});
				window["lines2_" + slide].animate(window["line_g_" + slide], 1300, mina.easeinout, function() {});

				window["poly_" + slide].animate({
					d: window["d_" + slide]
